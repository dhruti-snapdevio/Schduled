...
import { user, userProfile, userBranding, usernameRedirect, notificationPreference, connectedCalendar, contact } from '@/db/schema'
...
export async function updateBranding(data: {
  ...
  try {
    ...
    if (displayName.length > 64) return { error: 'Display name must be 64 characters or less' }
...
export async function updateCommunicationPrefs(data: CommPrefs): Promise<ActionResult> {
  try {
    const session = await requireSession()
...
export async function disconnectCalendar(calendarId: string): Promise<ActionResult> {
  try {
    const session = await requireSession()
...
export async function reconnectCalendar(): Promise<ActionResult<{ redirectUrl: string }>> {
  try {
...
export async function upsertContactNote(
  ...
  try {
    ...
    const [existing] = await db
      ...
      .from(contact)
      .where(and(
        eq(contact.hostUserId, session.user.id),
...
export async function archiveContact(email: string, name: string): Promise<ActionResult> {
  try {
    const session = await requireSession()
