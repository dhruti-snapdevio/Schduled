...
| `ACTIVITY_LOG_RETENTION_DAYS` | Opt | unset (forever) | How many days of `audit_logs` rows to keep — a nightly job (`platform.audit-logs-prune`) deletes anything older. Unset = never prune; you own the Postgres instance, unlike a SaaS provider's storage-cost-driven cap. | Set a number, e.g. `180`, if you want automatic pruning. |
