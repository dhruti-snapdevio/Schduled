...
Calendly's "Scheduled Events" tab is the direct equivalent:
- List view of all events (upcoming + past)
- Filter by event type, date, invitee email
- Click any event to see full details + invitee responses
- Host can cancel from the detail view
- Status badges: Active, Cancelled

Schedica improves on this with:
- Post-meeting notes
- No-Show status tracking
- Search by invitee name
- Quicker access to video join link from the list
